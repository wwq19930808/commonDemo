<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
         ...
         <el-form-item>
                  <el-select  v-model="filters.roadId" placeholder="道路名称">
                   <el-option
                ...
                <el-form-item >
                  <el-select  v-model="filters.eleId" placeholder="元素名称">
                   <el-option
                       v-for="item in eleId"
                      :key="item.blockId"
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="roadName" label="道路名称" min-width="140" sortable> 
			</el-table-column>
			<el-table-column prop="eleName" label="元素名称" min-width="140"  sortable>
			</el-table-column>
			<el-table-column prop="eleTypeName" label="道路元素" min-width="140" sortable>
			</el-table-column>
			<el-table-column prop="sortNo" label="排序序号" min-width="140" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form  :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
                ...
                <el-form-item label="元素名称" prop="eleId">
                  <el-select :size="'mini'" v-model="editForm.eleId" placeholder="请选择">
                   <el-option
                       v-for="item in eleId"
                      :key="item.blockId"
                      :label="item.blockName"
				...
				<el-form-item label="道路元素" prop="eleTypeCode">
                  <el-select :size="'mini'" v-model="editForm.eleTypeCode" placeholder="请选择">
                   <el-option
                       v-for="item in eleTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
			 <el-form-item label="道路名称" prop="roadId">
                ...
                <el-form-item label="元素名称" prop="eleId">
                  <el-select :size="'mini'" v-model="addForm.eleId" placeholder="请选择">
                   <el-option
                       v-for="item in eleId"
                      :key="item.dicCode"
                      :label="item.eleName"
				...
				<el-form-item label="道路元素" prop="eleTypeCode">
                  <el-select :size="'mini'" v-model="addForm.eleTypeCode" placeholder="请选择">
                   <el-option
                       v-for="item in eleTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.eleId = this.filters.eleId;
      para.roadId = this.filters.roadId;
      this.listLoading = true;

      getPageRoadElement({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { roadId: row.roadId ,eleId: row.eleId };
          console.log(para);
          removeRoadElement({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editRoadElement({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addRoadElement({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
    ...
    batchRemove: function() {
        ...
        if ((resp.status = 200 && !!resp.data)) {
          resp.data.resultList.map((item, index) => {
            switch (item.parentCode) {
                case "TRFLOWPOSITION":
               	eleTypeCode.push(item);
                break;    
              default:
                break;
            }
          });
           this.eleTypeCode=eleTypeCode;
     ...
     getElement: function() {
      ...
      getElement({ data: JSON.stringify(param) }).then(resp => {
        console.log(resp);
        if ((resp.status = 200 && !!resp.data)) {
          this.eleId = resp.data.resultList;
