	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.rltId" placeholder="关联编码"></el-input>
				</el-form-item>
				<el-form-item>
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="rltId" label="关联编码" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="blockId" label="路段" min-width="120"  sortable>
			</el-table-column>
			<el-table-column prop="rltBlockId" label="对应路段" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="rltModeName" label="关联方式" min-width="120" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
               ...
               <el-form-item label="对应路段" prop="rltBlockId">
                  <el-select :size="'mini'" v-model="editForm.rltBlockId" placeholder="请选择">
                   <el-option
                       v-for="item in rltBlockId"
                      :key="item.rltBlockId"
                      :label="item.blockName"
                      :value="item.blockId">
                   </el-option>
				...
				<el-form-item label="关联方式" prop="rltModeCode">
                  <el-select :size="'mini'" v-model="editForm.rltModeCode" placeholder="请选择">
                   <el-option
                       v-for="item in rltModeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			...
			<el-form-item label="路段编码" prop="blockId">
               ...
               <el-form-item label="对应路段" prop="rltBlockId">
                  <el-select :size="'mini'" v-model="addForm.rltBlockId" placeholder="请选择">
                   <el-option
                       v-for="item in rltBlockId"
                      :key="item.rltBlockId"
                      :label="item.blockName"
				...
				<el-form-item label="关联方式" prop="rltModeCode">
                  <el-select :size="'mini'" v-model="addForm.rltModeCode" placeholder="请选择">
                   <el-option
                       v-for="item in rltModeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      getPageBlockBlock({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          console.log(para);
          removeBlockBlock({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editBlockBlock({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addBlockBlock({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
    ...
    batchRemove: function() {
        ...
        if ((resp.status = 200 && !!resp.data)) {
          resp.data.resultList.map((item, index) => {
            switch (item.parentCode) {
                case "BLOCKRELATION":
               	rltModeCode.push(item);
                break;    
              default:
                break;
            }
          });
           this.rltModeCode=rltModeCode;
     ...
     getRltBlockInfo: function() {
         let param = {
         
           rltBlockId:"",

      };
      getRltBlockInfo({ data: JSON.stringify(param) }).then(resp => {
        console.log(resp);
        if ((resp.status = 200 && !!resp.data)) {
          this.rltBlockId = resp.data.resultList;
