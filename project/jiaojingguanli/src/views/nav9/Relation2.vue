	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.laneId" placeholder="车道编码"></el-input>
				</el-form-item>
          <el-form-item >
                  <el-select  v-model="filters.crossId" placeholder="所属路口">
                   <el-option
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="laneId" label="车道编码" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="crossName" label="所属路口" min-width="120"  sortable>
			</el-table-column>
			<el-table-column prop="sortNo" label="车道序号" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="laneAttrName" label="车道属性" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="directName" label="入口车道方向" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="trFlowDirectName" label="入口交通流向" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="laneFeatureName" label="车道特征" min-width="120" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="所属路口" prop="crossId">
                  <el-select :size="'mini'" v-model="editForm.crossId" placeholder="请选择">
                   <el-option
					...
					<el-form-item label="车道编码" prop="laneId">
					<el-input v-model="editForm.laneId" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="车道序号" prop="sortNo">
					<el-input v-model="editForm.sortNo"></el-input>
				</el-form-item>
				<el-form-item label="车道属性" prop="laneAttrCode">
                  <el-select :size="'mini'" v-model="editForm.laneAttrCode" placeholder="请选择">
                   <el-option
                       v-for="item in laneAttrCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
               ...
               <el-form-item label="入口方向" prop="directCode">
				 ...
				 <el-form-item label="交流流向" prop="trFlowDirectCode">
                  <el-select :size="'mini'" v-model="editForm.trFlowDirectCode" placeholder="请选择">
                   <el-option
                       v-for="item in trFlowDirectCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
				 ...
				 <el-form-item label="车道特征" prop="laneFeatureCode">
                  <el-select :size="'mini'" v-model="editForm.laneFeatureCode" placeholder="请选择">
                   <el-option
                       v-for="item in laneFeatureCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			...
			<el-form-item label="所属路口" prop="crossId">
				...
				<el-form-item label="车道属性" prop="laneAttrCode">
                  <el-select :size="'mini'" v-model="addForm.laneAttrCode" placeholder="请选择">
                   <el-option
                       v-for="item in laneAttrCode"
                      :key="item.dicCode"
                      :label="item.dicName"
               ...
               <el-form-item label="入口方向" prop="directCode">
				 ...
				 <el-form-item label="交流流向" prop="trFlowDirectCode">
                  <el-select :size="'mini'" v-model="addForm.trFlowDirectCode" placeholder="请选择">
                   <el-option
                       v-for="item in trFlowDirectCode"
                      :key="item.dicCode"
                      :label="item.dicName"
				 ...
				 <el-form-item label="车道特征" prop="laneFeatureCode">
                  <el-select :size="'mini'" v-model="addForm.laneFeatureCode" placeholder="请选择">
                   <el-option
                       v-for="item in laneFeatureCode"
                      :key="item.dicCode"
                      :label="item.dicName"
...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        laneId: [{ required: true, message: "请输入内容", trigger: "blur" }],
        crossId: [
  ...
  methods: {
    ...
    getUsers() {
      ...
      getPageCrossLane({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { crossId: row.crossId ,laneId: row.laneId};
          console.log(para);
          removeCrossLane({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editCrossLane({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addCrossLane({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
        ...
        if ((resp.status = 200 && !!resp.data)) {
          resp.data.resultList.map((item, index) => {
            switch (item.parentCode) {
                ...
                break;
                case "LANEATTR":
               	laneAttrCode.push(item);
                break;
                case "TRFLOWDIRECT":
               	trFlowDirectCode.push(item);
                break;
                case "LANEFEATURE":
               	laneFeatureCode.push(item);
                break;
