<template>
	<section>
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="deviceName" label="设施名称" width="120" sortable>
			</el-table-column>
			<el-table-column prop="deviceTypeName" label="设施类型" width="150"  sortable>
			</el-table-column>
			<el-table-column prop="brand" label="设施品牌" width="130" sortable>
			</el-table-column>
			<el-table-column prop="material" label="设施材料" width="130" sortable>
			</el-table-column>
			<el-table-column prop="factoryDate" label="出场时间" width="130" sortable>
			</el-table-column>
			<el-table-column prop="storageDate" label="购入时间" width="130" sortable>
			</el-table-column>
			<el-table-column prop="usageState" label="使用状态" width="150" sortable>
			</el-table-column>			
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			...
			<el-form-item label="设备类别" prop="deviceType">
				...
				<el-form-item label="购入日期">
					<el-date-picker type="date" placeholder="选择日期" v-model="addForm.storageDate"></el-date-picker>
				</el-form-item>
				<el-form-item label="出厂日期">
...
import {
 getPageTrafficDevInfo,
  // batchRemoveUser,
 addTrafficDevInfo,
  editTrafficDevInfo,
  removeTrafficDevInfo,
  getDicInfo,
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      getPageTrafficDevInfo({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          console.log(para);
          removeTrafficDevInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editTrafficDevInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addTrafficDevInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
