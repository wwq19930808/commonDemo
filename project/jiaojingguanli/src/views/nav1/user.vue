	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.regionName" placeholder="区划名称"></el-input>
				</el-form-item>
        <el-form-item>
          <el-input v-model="filters.regionId" placeholder="区划编码"></el-input>
        </el-form-item>
         <el-form-item >
                  <el-select  v-model="filters.parentId" placeholder="上级区划">
                   <el-option
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="regionId" label="区划编码" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="regionName" label="区划名称" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="regionTypeName" label="区划类型" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="parentName" label="上级区划" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="area" label="面积(平方公里)" min-width="120" sortable>
			</el-table-column>
			<el-table-column prop="coordX,coordY" label="中心位置" min-width="180" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="区划编码" prop="regionId">
					<el-input v-model="editForm.regionId" auto-complete="off"></el-input>
				</el-form-item>
				...
				<el-form-item label="区划类型" prop="regionTypeCode">
					<el-select :size="'mini'" v-model="editForm.regionTypeCode" placeholder="请选择">
                   <el-option
                      v-for="item in regionTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
        ...
        <el-form-item label="上级区划" prop="parentId">
                  <el-select :size="'mini'" v-model="editForm.parentId" placeholder="请选择">
                   <el-option
                       v-for="item in parentId"
                      :key="item.regionId"
                      :label="item.regionName"
				...
				<el-form-item label="中心位置" prop="coordX">
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm" width="193px">
				<el-form-item label="区划编码" prop="regionId">
					<el-input v-model="addForm.regionId" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="区划名称" prop="regionName">
					<el-input v-model="addForm.regionName" auto-complete="off"></el-input>
				</el-form-item>
				...
				<el-form-item label="区划类型" prop="regionTypeCode">
					<el-select :size="'mini'" v-model="addForm.regionTypeCode" placeholder="请选择">
                   <el-option
                      v-for="item in regionTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
         ...
         <el-form-item label="上级区划" prop="parentId">
                  <el-select :size="'mini'" v-model="addForm.parentId" placeholder="请选择">
                   <el-option
                       v-for="item in parentId"
                      :key="item.regionId"
                      :label="item.regionName"
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.regionName = this.filters.regionName;
      para.regionId = this.filters.regionId;
      para.parentId = this.filters.parentId;
      this.listLoading = true;
      getRegionListPage({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { regionId: row.regionId };
          console.log(para);
          removeRegionInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editRegionInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addRegionInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
    ...
    getSelectSource: function() {
     ...
     getRegionInfo: function() {
      ...
      getRegionInfo({ data: JSON.stringify(param) }).then(resp => {
        console.log(resp);
        if ((resp.status = 200 && !!resp.data)) {
          this.parentId = resp.data.resultList;
