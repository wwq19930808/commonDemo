	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.tslId" placeholder="设备编码"></el-input>
				</el-form-item>
        <el-form-item>
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="tslId" label="信号灯编码" width="130" sortable>
			</el-table-column>
			<el-table-column prop="tsName" label="点位名称" width="130"  sortable>
			</el-table-column>
			<el-table-column prop="directName" label="信号灯方向" width="140" sortable>
			</el-table-column>
			<el-table-column prop="deviceStatusName" label="设备状态" width="150" sortable>
			</el-table-column>
			<el-table-column prop="sortNo" label="信号灯排序" width="140" sortable>
			</el-table-column>
			<el-table-column prop="roadName" label="所在道路" width="120" sortable>
			</el-table-column>
			<el-table-column prop="blockName" label="所在路段" width="120" sortable>
			</el-table-column>
			<el-table-column prop="laneId" label="所在车道" width="130" sortable>
			</el-table-column>
			<el-table-column prop="isVariableName" label="车道可变" min-width="120" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="信号灯编码" prop="tslId">
					<el-input v-model="editForm.tslId"></el-input>
				</el-form-item>
				 <el-form-item label="信号灯朝向" prop="directCode">
				...
				<el-form-item label="信号灯排序" prop="sortNo">
					<el-input v-model="editForm.sortNo"></el-input>
				</el-form-item>
				<el-form-item label="点位名称" prop="tsId">
                  <el-select :size="'mini'" v-model="editForm.tsId" placeholder="请选择">
                   <el-option
                       v-for="item in tsId"
                      :key="item.tsId"
                      :label="item.tsName"
               ...
               <el-form-item label="所在道路" prop="roadId">
                ...
                <el-form-item label="车道可变" prop="isVariableCode">
				...
				<el-form-item label="所在车道" prop="laneId">
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="信号灯编码" prop="tslId">
					<el-input v-model="addForm.tslId"></el-input>
				</el-form-item>
				 <el-form-item label="信号灯朝向" prop="directCode">
				...
				<el-form-item label="点位名称" prop="tsId">
                  <el-select :size="'mini'" v-model="addForm.tsId" placeholder="请选择">
                   <el-option
                       v-for="item in tsId"
                      :key="item.tsId"
                      :label="item.tsName"
...
import {
 getPageTideSigLampInfo,
  // batchRemoveUser,
 addTideSigLampInfo,
  editTideSigLampInfo,
  removeTideSigLampInfo,
  getDicInfoDob,
  getDicInfo,
  getTideSiteInfo,
  getRoadInfo,
...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        tslId: [{ required: true, message: "请输入内容", trigger: "blur" }],
        directCode: [
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.tslId = this.filters.tslId;
      this.listLoading = true;
      getPageTideSigLampInfo({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { tslId: row.tslId };
          console.log(para);
          removeTideSigLampInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editTideSigLampInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addTideSigLampInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
    ...
    batchRemove: function() {
     ...
     getTideSiteInfo: function() {
         let param = {
       tsId: "",
     	
     	
      };
      getTideSiteInfo({ data: JSON.stringify(param) }).then(resp => {
        console.log(resp);
        if ((resp.status = 200 && !!resp.data)) {
          this.tsId = resp.data.resultList;
