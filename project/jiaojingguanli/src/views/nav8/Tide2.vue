	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.tsId" placeholder="点位编码"></el-input>
				</el-form-item>
        <el-form-item>
          <el-input v-model="filters.tsName" placeholder="点位名称"></el-input>
        </el-form-item>
         <el-form-item >
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="tsId" label="点位编码" width="130" sortable>
			</el-table-column>
			<el-table-column prop="tsName" label="点位名称" width="130"  sortable>
			</el-table-column>
			<el-table-column prop="ip" label="IP" width="140" sortable>
			</el-table-column>
			<el-table-column prop="sigLampAmount" label="车道信号灯数量" width="160" sortable>
			</el-table-column>
			<el-table-column prop="directName" label="信息灯朝向" width="140" sortable>
			</el-table-column>
			...
			<el-table-column prop="deptName" label="设备管理部门" width="150" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="点位编码" prop="tsId">
					<el-input v-model="editForm.tsId" auto-complete="off"></el-input>
				</el-form-item>
					<el-form-item label="点位名称" prop="tsName">
					<el-input v-model="editForm.tsName" auto-complete="off"></el-input>
				</el-form-item>
				 <el-form-item label="设备状态" prop="deviceStatusCode">
                  <el-select :size="'mini'" v-model="editForm.deviceStatusCode" placeholder="请选择">
                   <el-option
					...
					<el-form-item label="信号灯数" prop="sigLampAmount">
					<el-input v-model="editForm.sigLampAmount" ></el-input>
				</el-form-item>
				...
				<el-form-item label="管理部门" prop="manageDept">
                  <el-select :size="'mini'" v-model="editForm.manageDept" placeholder="请选择">
                   <el-option
                       v-for="item in manageDept"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="点位编码" prop="tsId">
					<el-input v-model="addForm.tsId" auto-complete="off"></el-input>
				</el-form-item>
					<el-form-item label="点位名称" prop="tsName">
					<el-input v-model="addForm.tsName" auto-complete="off"></el-input>
				</el-form-item>
				 <el-form-item label="设备状态" prop="deviceStatusCode">
                  <el-select :size="'mini'" v-model="addForm.deviceStatusCode" placeholder="请选择">
                   <el-option
					...
					<el-form-item label="信号灯数" prop="sigLampAmount">
					<el-input v-model="addForm.sigLampAmount" ></el-input>
				</el-form-item>
				<el-form-item label="信号灯朝向" prop="directCode">
...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        tsId: [{ required: true, message: "请输入内容", trigger: "blur" }],
        tsName: [
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.tsName = this.filters.tsName;
      para.tsId = this.filters.tsId;
      para.deviceStatusCode = this.filters.deviceStatusCode;
      this.listLoading = true;

      getPageTideSiteInfo({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { tsId: row.tsId };
          console.log(para);
          removeTideSiteInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editTideSiteInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addTideSiteInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
