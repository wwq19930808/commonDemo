	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.orgId" placeholder="机构代码"></el-input>
				</el-form-item>
        <el-form-item>
          <el-input v-model="filters.orgName" placeholder="机构名称"></el-input>
        </el-form-item>
        <el-form-item >
          <el-select  v-model="filters.orgTypeCode" placeholder="机构类型">
                   <el-option
                      v-for="item in orgTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="orgId" label="机构代码" width="120" sortable>
			</el-table-column>
			<el-table-column prop="orgName" label="机构名称" width="120"  sortable>
			</el-table-column>
			<el-table-column prop="orgTypeName" label="机构类型" width="120" sortable>
			</el-table-column>
			<el-table-column prop="addr" label="机构地址" width="180" sortable>
			</el-table-column>
			...
			<el-table-column prop="businessScope" label="主要业务范围" width="180" sortable>
			</el-table-column>
			<el-table-column prop="memo" label="备注" min-width="80" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="机构类型" prop="orgTypeCode">
					<el-select :size="'mini'" v-model="editForm.orgTypeCode" placeholder="请选择">
                   <el-option
                      v-for="item in orgTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
				...
				<el-form-item label="注册时间" prop="regTime">
					<el-date-picker type="date" placeholder="选择日期" v-model="editForm.regTime"></el-date-picker>
				</el-form-item>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				...
				<el-form-item label="机构类型" prop="orgTypeCode">
					<el-select :size="'mini'" v-model="addForm.orgTypeCode" placeholder="请选择">
                   <el-option
                      v-for="item in orgTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
				...
				<el-form-item label="注册时间" prop="regTime">
					<el-date-picker type="date" placeholder="选择日期" v-model="addForm.regTime"></el-date-picker>
				</el-form-item>
				<el-form-item label="业务范围" prop="businessScope">
...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        orgId: [{ required: true, message: "请输入内容", trigger: "blur" }],
        orgName: [
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.orgName = this.filters.orgName;
      para.orgId = this.filters.orgId;
      para.orgTypeCode = this.filters.orgTypeCode;
      this.listLoading = true;
      getPageOrganInfo({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          console.log(para);
          removeOrganInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editOrganInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addOrganInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
