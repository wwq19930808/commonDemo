	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.matrixId" placeholder="矩阵编码"></el-input>
				</el-form-item>
        <el-form-item>
          <el-input v-model="filters.matrixName" placeholder="矩阵名称"></el-input>
        </el-form-item>
         <el-form-item >
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="matrixId" label="矩阵编码" width="130" sortable>
			</el-table-column>
			<el-table-column prop="matrixName" label="矩阵名称" width="130"  sortable>
			</el-table-column>
			<el-table-column prop="matrixTypeName" label="矩阵类型" width="140" sortable>
			</el-table-column>
			<el-table-column prop="deviceStatusName" label="设备状态" width="130" sortable>
			</el-table-column>
			<el-table-column prop="channsIn" label="视频输入路数" width="150" sortable>
			</el-table-column>
			<el-table-column prop="channsOut" label="视频输入路数" width="150" sortable>
			</el-table-column>
			<el-table-column prop="manufact" label="生产厂家" width="140" sortable>
			</el-table-column>
			<el-table-column prop="model" label="型号名称" width="130" sortable>
			</el-table-column>
			<el-table-column prop="manageId" label="设备管理编码" width="150" sortable>
			</el-table-column>
			<el-table-column prop="deptName" label="设备管理部门" min-width="150" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="矩阵编码" prop="matrixId">
					<el-input v-model="editForm.matrixId" auto-complete="off"></el-input>
				</el-form-item>
					<el-form-item label="矩阵名称" prop="matrixName">
					<el-input v-model="editForm.matrixName" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="矩阵类型" prop="matrixTypeCode">
                  <el-select :size="'mini'" v-model="editForm.matrixTypeCode" placeholder="请选择">
                   <el-option
                       v-for="item in matrixTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
                      :value="item.dicCode">
                   </el-option>
               ...
               <el-form-item label="设备状态" prop="deviceStatusCode">
				...
				<el-form-item label="输入路数" prop="channsIn">
					<el-input v-model="editForm.channsIn"></el-input>
				</el-form-item>
				<el-form-item label="输出路数" prop="channsOut">
					<el-input v-model="editForm.channsOut"></el-input>
				</el-form-item>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true" :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="矩阵编码" prop="matrixId">
					<el-input v-model="addForm.matrixId" auto-complete="off"></el-input>
				</el-form-item>
					<el-form-item label="矩阵名称" prop="matrixName">
					<el-input v-model="addForm.matrixName" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="矩阵类型" prop="matrixTypeCode">
                  <el-select :size="'mini'" v-model="addForm.matrixTypeCode" placeholder="请选择">
                   <el-option
                       v-for="item in matrixTypeCode"
                      :key="item.dicCode"
                      :label="item.dicName"
               ...
               <el-form-item label="设备状态" prop="deviceStatusCode">
				...
				<el-form-item label="输入路数" prop="channsIn">
					<el-input v-model="addForm.channsIn"></el-input>
				</el-form-item>
				<el-form-item label="输出路数" prop="channsOut">
					<el-input v-model="addForm.channsOut"></el-input>
				</el-form-item>
				<el-form-item label="生产厂家" prop="manufact">
...
import {
 getPageVideoMatrixInfo,
  // batchRemoveUser,
 addVideoMatrixInfo,
  editVideoMatrixInfo,
  removeVideoMatrixInfo,
  getDicInfoDob,
  getDicInfo
...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        matrixId: [{ required: true, message: "请输入内容", trigger: "blur" }],
        matrixName: [
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.matrixName = this.filters.matrixName;
      para.matrixId = this.filters.matrixId;
      para.deviceStatusCode = this.filters.deviceStatusCode;
      this.listLoading = true;
      getPageVideoMatrixInfo({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { matrixId: row.matrixId };
          console.log(para);
          removeVideoMatrixInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editVideoMatrixInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addVideoMatrixInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
