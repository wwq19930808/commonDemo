	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				<el-form-item>
					<el-input v-model="filters.vcpId" placeholder="服务器编码"></el-input>
				</el-form-item>
        <el-form-item>
          <el-input v-model="filters.vcpName" placeholder="服务器名称"></el-input>
        </el-form-item>
        <el-form-item >
		...
		<el-table :data="newslist" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="vcpId" label="服务器编码" width="130" sortable>
			</el-table-column>
			<el-table-column prop="vcpName" label="服务器名称" width="130"  sortable>
			</el-table-column>
			...
			<el-table-column prop="version" label="当前版本" width="150" sortable>
			</el-table-column>
			<el-table-column prop="versionUpdTime" label="版本更新时间" width="140" sortable>
			</el-table-column>
		...
		<el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :inline="true"  :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				<el-form-item label="服务器编码" prop="vcpId">
					<el-input v-model="editForm.vcpId"></el-input>
				</el-form-item>
				<el-form-item label="服务器名称" prop="vcpName">
					<el-input v-model="editForm.vcpName"></el-input>
				</el-form-item>
		...
		<el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
			<el-form :inline="true"  :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
				<el-form-item label="服务器编码" prop="vcpId">
					<el-input v-model="addForm.vcpId"></el-input>
				</el-form-item>
				<el-form-item label="服务器名称" prop="vcpName">
					<el-input v-model="addForm.vcpName"></el-input>
				</el-form-item>
				<el-form-item label="管理部门" prop="manageDept">
...
import {
 getPageVideoCProxyInfo,
  // batchRemoveUser,
 addVideoCProxyInfo,
  editVideoCProxyInfo,
  removeVideoCProxyInfo,
  getDicInfoDob,
  getDicInfo
...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        vcpId: [{ required: true, message: "请输入内容", trigger: "blur" }],
        vcpName: [
  ...
  methods: {
    ...
    getUsers() {
      ...
      para.vcpName = this.filters.vcpName;
      para.vcpId = this.filters.vcpId;
      para.deviceStatusCode = this.filters.deviceStatusCode;
      this.listLoading = true;
      getPageVideoCProxyInfo({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          //NProgress.start();
          let para = { vcpId: row.vcpId };
          console.log(para);
          removeVideoCProxyInfo({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editVideoCProxyInfo({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addVideoCProxyInfo({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
