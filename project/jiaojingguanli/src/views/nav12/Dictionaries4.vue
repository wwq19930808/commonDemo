...
export default {
  data() {
    return {
      ...
      addLoading: false,
      addFormRules: {
        dicCode: [{ required: true, message: "请输入内容", trigger: "blur" }],
        personName: [
        ...
        itemCode: [{ required: true, message: "请输入内容", trigger: "blur" }],
        dicValue: [
          { required: true, message: "请输入内容", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    getUsers() {
      ...
      getPageVehicleDic({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          console.log(para);
          removeVehicleDic({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editVehicleDic({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addVehicleDic({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
