...
export default {
  data() {
    return {
      ...
      addFormRules: {
        ...
        dicName: [{ required: true, message: "请输入内容", trigger: "blur" }],
        parentCode: [
          { required: true, message: "请输入内容", trigger: "blur" }
        ],
  ...
  methods: {
    ...
    getUsers() {
      ...
      getPageTrafficDic({ data: JSON.stringify(para) }).then(res => {
        console.log(res);
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          console.log(para);
          removeTrafficDic({ data: JSON.stringify(para) }).then(res => {
            console.log(res);
            this.listLoading = false;
            //NProgress.done();
            this.$message({
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            editTrafficDic({ data: JSON.stringify(para) }).then(res => {
          ...
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            para.birthDay =
              !para.birthDay || para.birthDay == ""
                ? ""
                : util.formatDate.format(new Date(para.birthDay), "yyyy-MM-dd");
            addTrafficDic({ data: JSON.stringify(para) }).then(res => {
              console.log(res);
